...
from utils import (
    ...
    from_role_to_role_array
)

GUILD_CONTRACT = os.path.join("contracts/access_control", "GuildContractAccess.cairo")
GUILD_MANAGER = os.path.join("contracts/access_control", "GuildManagerAccess.cairo")
GUILD_CERTIFICATE = os.path.join("contracts/access_control", "GuildCertificateAccess.cairo")
...
async def contract_factory():
    ...
    roles = [
        (
            1,
            [
                get_selector_from_name("execute_transactions"),
                get_selector_from_name("remove_member"),
                # get_selector_from_name("deposit"),
                # get_selector_from_name("withdraw"),
                # get_selector_from_name("update_role")
            ],
        ),
        (
            2,
            [
                # get_selector_from_name("whitelist_member")
                get_selector_from_name("remove_member")
            ]
        )
    ]

    (role_array, selectors) = from_role_to_role_array(roles)

    print(
        len(role_array),
        *[x for t in role_array for x in t],
        len(selectors),
        *selectors
    )
    ...
    guild_class = compile_starknet_files([GUILD_CONTRACT], debug_info=True)
