...
func set_dummy_value{
        ...
        range_check_ptr
    }(token_gated_account_address:felt, target_address:felt, val:felt) -> ():

    alloc_locals
    let (__fp__, _) = get_fp_and_pc()
    let (nonce)=IAccount.get_nonce(token_gated_account_address)
   ...
   # [call_array].len=1
    ...
    let (response_len,response)= IAccount.__execute__(token_gated_account_address,
                                                call_array_len,
